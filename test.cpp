...
template<typename T>
void compare_output(T result, T expected, int test_num, string problem = ""){

    // Compare expected output with result
    if(expected == result){

        // Test passed
        cout << problem << " Test " << test_num << " Passed\n";
    } else {

        // Test failed
       cout << problem << " Test " << test_num << " Failed\n";
    }
...
void testLostParenthesis(T lp){
    ...
    compare_output(lp.minResult("55-66-77-88-99"), -275, 2, "Lost Parenthesis");

    // Test 3: Mix of positive and negative numbers
    compare_output(lp.minResult("55-66+77-88+99"), -275, 3, "Lost Parenthesis");

    // Test 4: Single number
    compare_output(lp.minResult("55"), 55, 4, "Lost Parenthesis");

    // Test 5 digit number
    compare_output(lp.minResult("00009-00009"), 0, 5, "Lost Parenthesis");

    // Test 6 One minus at the start
    compare_output(lp.minResult("55-66+77+88+99"), -275, 6, "Lost Parenthesis");

    // Test 7 One minus at the end
    compare_output(lp.minResult("55+66+77+88-99"), 187, 7, "Lost Parenthesis");

    // Test 8 50 character input expression
    compare_output(lp.minResult("00009+00009+00009+00009+00009+00009+00009+0009+009"), 81, 8, "Lost Parenthesis");
...
void testAVLTree(T tree){
    ...
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D3 IN"), string("1 2 4 5 "), 2, "AVL Tree");
    tree.processInput("D1 D2 D4 D5");

    // Test 3: Insert Duplicate elements
    compare_output(tree.processInput("A1 A2 A3 A4 A5 A5 A5 IN"), string("1 2 3 4 5 "), 3, "AVL Tree");
    tree.processInput("D1 D2 D3 D4 D5");

    // Test 4: Delete element not in tree
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D6 IN"), string("1 2 3 4 5 "), 4, "AVL Tree");
    tree.processInput("D1 D2 D3 D4 D5");

    // Test 5: Insert 5 elements and delete 5 elements
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D1 D2 D3 D4 D5 IN"), string("EMPTY"), 5, "AVL Tree");

    // Test 6: Delete duplicate elements
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D5 D5 D5 IN"), string("1 2 3 4 "), 6, "AVL Tree");
    tree.processInput("D1 D2 D3 D4");

    // Test 7: Delete elements from empty tree
    compare_output(tree.processInput("D1 D2 D3 D4 D5 IN"), string("EMPTY"), 7, "AVL Tree");

    // Test 8: Insert 5 elements - Display Preorder
    compare_output(tree.processInput("A1 A2 A3 A4 A5 PRE"), string("2 1 4 3 5 "), 8, "AVL Tree");
    tree.processInput("D1 D2 D3 D4 D5");

    // Test 9: Insert 5 elements - Display Postorder
    compare_output(tree.processInput("A1 A2 A3 A4 A5 POST"), string("1 3 5 4 2 "), 9, "AVL Tree");
    tree.processInput("D1 D2 D3 D4 D5");

    // Test 10: Insert 5 elements and delete 1 element - Display Preorder
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D3 PRE"), string("2 1 4 5 "), 10, "AVL Tree");
    tree.processInput("D1 D2 D3 D4");

    // Test 11: Insert 5 elements and delete 1 element - Display Postorder
    compare_output(tree.processInput("A1 A2 A3 A4 A5 D3 POST"), string("1 5 4 2 "), 11, "AVL Tree");
    tree.processInput("D1 D2 D3 D4");

    // Test 12: Large input - Inorder
    string large_input = string("D13 D60 D76 D12 A17 D98 A94 D70 D3 A23 A42 D45 A100 ")
    ...
    string expected_output = string("4 6 9 12 13 15 17 19 20 22 23 24 25 27 35 39 40 42 44 45 ")
        + "48 50 58 60 63 71 72 73 75 81 82 87 90 91 97 98 ";

    compare_output<string>(tree.processInput(large_input), expected_output, 12, "AVL Tree");

    string clean = string("D4 D6 D9 D12 D13 D15 D17 D19 D20 D22 D23 D24 D25 D27 D35 D39 D40 D42 D44 D45 ")
    ...
    compare_output(tree.processInput(large_input), expected_output, 13, "AVL Tree");
    ...
    compare_output(tree.processInput(large_input), expected_output, 14, "AVL Tree");
